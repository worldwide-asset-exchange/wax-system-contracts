...
BOOST_FIXTURE_TEST_CASE(voter_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // producvotera is the only voter and should get all staker rewards
   {
      ...
      BOOST_REQUIRE_EQUAL(int64_t(new_tokens * 4 / 10), balance.get_amount() - initial_balance.get_amount());
   }
...
BOOST_FIXTURE_TEST_CASE(multiple_voters, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // Validate same reward for voters in the same condition.
   {
      ...
      uint64_t voters_inflation = 4 * (mid_term_supply.get_amount() - initial_supply.get_amount()) / 10
         + 4 * (supply.get_amount() - mid_term_supply.get_amount()) / 10;
...
BOOST_FIXTURE_TEST_CASE(producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      int64_t from_perblock_bucket = int64_t( initial_supply.get_amount() * double(secs_between_fills) * (continuous_rate * 3. / 10.) / secs_per_year ) ;
   ...
   // wait 5 more minutes, defproducera can now claim rewards again
   {
      ...
      BOOST_REQUIRE(5 > ((supply.get_amount() - initial_supply.get_amount()) - ((supply.get_amount() - initial_supply.get_amount()) * 3 / 10) - ((supply.get_amount() - initial_supply.get_amount()) * 4 / 10)) -
                          (savings - initial_savings));

      int64_t to_producer        = int64_t( (double(initial_supply.get_amount()) * double(usecs_between_fills) * continuous_rate) / usecs_per_year ) * 3 / 10;
...
BOOST_FIXTURE_TEST_CASE(adjust_chain_inflation, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // Simulates a scenario where the tokenomics can adjust the inflation rate.
   {
      ...
      BOOST_REQUIRE_EQUAL(int64_t(distribute_tokens - (distribute_tokens * 3 / 10) - (distribute_tokens * 4 / 10)), savings - initial_savings);
      BOOST_REQUIRE_EQUAL(int64_t(distribute_tokens * 3 / 10), balance.get_amount() - initial_balance.get_amount());
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      const int64_t expected_perblock_bucket = int64_t( double(initial_supply.get_amount()) * double(usecs_between_fills) * (cont_rate * 3. / 10.) / usecs_per_year );
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   // The test below is invalid now, the check top_producers.size() < _gstate.last_producer_schedule_size removed
...
BOOST_FIXTURE_TEST_CASE( wcap_017_setramrate_ceiling_and_supply_wrap, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( ceiling, get_global_state2()["new_ram_per_block"].as<uint16_t>() );

   // Accrual: no RAM-market action for 80 hours, then one buyram credits exactly slots * rate.
   const uint64_t before = get_global_state()["max_ram_size"].as_uint64();
   const auto t0 = fc::time_point::from_iso_string( get_global_state2()["last_ram_increase"].as_string() );
   produce_block( fc::hours(80) );
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( success(), buyram( "alice1111111", "alice1111111", core_sym::from_string("10.0000") ) );
   const auto t1 = fc::time_point::from_iso_string( get_global_state2()["last_ram_increase"].as_string() );
   const uint64_t slots = uint64_t( (t1 - t0).count() / 500'000 );
   BOOST_REQUIRE_GT( slots * ceiling, uint64_t(1) << 32 );   // the case does exercise the old wrap
   BOOST_REQUIRE_EQUAL( before + slots * ceiling, get_global_state()["max_ram_size"].as_uint64() );

   // Whoever was authorised before still is; nobody else is.
   BOOST_REQUIRE_EQUAL( error("missing authority of eosio"),
                        push_action( "alice1111111"_n, "setramrate"_n, mvo()("bytes_per_block", uint16_t(0)) ) );
} FC_LOG_AND_RETHROW()
